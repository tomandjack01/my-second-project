...
# Add GraphExp to path for imports
sys.path.insert(0, osp.join(osp.dirname(osp.abspath(__file__)), '..', 'GraphExp'))
...
def pretrain(model, train_loader, optimizer, device, epoch, logger):
    model.train()
    loss_list = []
...
def main(cfg):
    ...
    train_idx = torch.arange(len(graphs)).tolist()
...
if __name__ == "__main__":
    ...
    # optimized_params = {}
    SOLVER_params = {}
    for key, value in cfg.SOLVER.items():  # type: ignore[attr-defined]
        param_type = type(value)
        sp = optimized_params.get(key, value)
        if type(sp) == float and param_type == int:
            sp = int(sp)
        SOLVER_params[key] = sp
    cfg.SOLVER.update(SOLVER_params)  # type: ignore[attr-defined]
    MODEL_params = {}
    for key, value in cfg.MODEL.items():  # type: ignore[attr-defined]
        param_type = type(value)
        sp = optimized_params.get(key, value)
        if type(sp) == float and param_type == int:
            sp = int(sp)
        MODEL_params[key] = sp
    cfg.MODEL.update(MODEL_params)  # type: ignore[attr-defined]

    print('---new cfg------')
    print(cfg)
    f1 = main(cfg)
    nni.report_final_result(float(f1))
